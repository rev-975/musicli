...
//        } while (next == current_playing_index && song_count > 1);
//        current_playing_index = next;
//    } else {
//        if (current_playing_index < song_count - 1) {
...
//        int prev;
//        do {
//            prev = rand() % song_count;
//        } while (prev == current_playing_index && song_count > 1);
//        current_playing_index = prev;
//    } else {
//        if (current_playing_index > 0) {
//            current_playing_index--;
//        } else if (repeat_mode) {
//            current_playing_index = song_count - 1;
//        } else {
//            return;
//        }
//    }
...
void handle_keypress(int ch) {
    ...
    if (search_active) {
        if (ch == 27) { // esc
            ...
            search_len = 0;
            search_query[0] = '\0';
        } else if (ch == '\n') { // play song from filtered list
            if (match_count > 0) {
                current_playing_index = match_indices[cursor_index];
                play_song(current_playing_index);
            }
    ...
    switch (ch) {
        ...
        case '\n': if (match_count > 0) { current_playing_index = match_indices[cursor_index]; play_song(current_playing_index); } break;
