...
void draw_ui() {
    ...
    if (selected_index >= 0 && selected_index < song_count) {
        ...
        mvprintw(0, 0, "currently playing: %s [%s/%s]", 
                 filenames[selected_index], elapsed, total);
    } else {
    ...
    int offset = 0;
    if (selected_index >= max_rows) offset = selected_index - max_rows + 1;

    for (int i = 0; i < max_rows && (i + offset) < song_count; i++) {
        int idx = i + offset;
        if (idx == selected_index) attron(A_REVERSE);
        mvprintw(start_row + i, 2, "%s", filenames[idx]);
        if (idx == selected_index) attroff(A_REVERSE);
    }
